...
async def fastpurger(purg):
    ...
    if msgs:
        ...
        \nJumlah Kenangan Yang Dihapus {str(count)} Kenangan")
    """
    if BOTLOG:
        await purg.client.send_message(
            BOTLOG_CHATID,
            "Suksess Menghapus Kenangan " + str(count) + " Kenangan Berhasil  Dibersihkan.")
    """
    await sleep(2)
    await done.delete()
...
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Suksess Menghapus Kenangan,` " + str(count) + " `Kenangan Telah Dihapus :)`",
    )
    """
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`Anda Telah Menghapus Kenangan,` " + str(count) + " Kenangan Berhasil Dihapus :)`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
...
async def delete_it(delme):
    ...
    if delme.reply_to_msg_id:
        try:
            ...
            if BOTLOG:
                await delme.client.send_message(
                    BOTLOG_CHATID, "`Anda Berhasil Menghapus Kenangan :)`")
            """
