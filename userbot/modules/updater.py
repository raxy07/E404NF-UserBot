...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await event.edit(
                f'{txt}\n`Kredensial Heroku tidak valid untuk deploy E404NF Userbot dyno.`'
            )
            return repo.__del__()
        await event.edit('`[HEROKU]:'
                         '\nPembaruan E404NF Userbot Sedang Dalam Prosess, Mohon Menunggu 7-8 Menit`'
                         )
        ups_rem.fetch(ac_br)
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n"
                "`E404NF-UserBot Berhasil Di Update`")
...
async def update(event, repo, ups_rem, ac_br):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n"
            "**E404NF-Userbot Telah Di Perbarui **")
...
async def upstream(event):
    ...
    if changelog == '' and force_update is False:
        await event.edit(
            f'\n**☫ E404NF Userbot Sudah Versi Terbaru**\n')
        await asyncio.sleep(15)
        await event.delete()
        return repo.__del__()
